...
const FAQItem = ({item, onClick, isOpen}) => {
  ...
  return (
    ...
    dark:hover:to-transparent' id={`faq-item-${item.id}`}>
      ...
      ${isOpen ? 'bg-gradient-to-r from-indigo-200 via-purlpe-200 to-indigo-200/90 dark:from-indigo-900/80 dark:via-purple-900/80 dark:to-indigo-900/70 text-purple-700 dark:text-white font-medium' : 
      ...
      onClick={() => onClick(item.id)}>
        ...
        w-8 min-8 aspect-square rounded-full ${isOpen ? 
      ...
      <div className='overflow-hidden transition-all duration-200 ease-out' id={`answer-${item.id}`} ref={answerRef}
      style={{maxHeight: isOpen ? answerRef.current.scrollHeight : 0, 
      opacity: isOpen ? 1 : 0 }}>
